...
graph LR
    ...
    subgraph DataInputs
        ...
        C[Activity Data kWh, Miles] --> D[Specific Factor];
        SUPPLIER_DATA[Supplier Emission Data Input] --> D;
    end

    subgraph CalculationScopes
    ...
    A --> |Initial S3 Estimate Spend| S3;
    B --> |Refines S3 Estimate| S3;
    C --> |Activity Data Input| S1;
    C --> |Activity Data Input| S3;
    ...
    SME --> S2;
    SME --> S3;
    S1 & S2 & S3 --> INTERNAL_REPORT[GHG Protocol Internal Report];
    INTERNAL_REPORT --> EXTERNAL_REPORTS[External Stakeholder Reports];

    %% Final Flow Details
    EXTERNAL_REPORTS --> |For Compliance| CLIENTS[Clients For Scope 3];
    EXTERNAL_REPORTS --> |For Financing| BANKS[Banks and Lenders];
    
    %% Style the SME Node to highlight the target
    style SME fill:#2E8B57,stroke:#3C3C3C,stroke-width:4px,color:#FFFFFF
    style EXTERNAL_REPORTS fill:#FFA500,stroke:#3C3C3C,stroke-width:2px,color:#333333
    style INTERNAL_REPORT fill:#ADD8E6,stroke:#3C3C3C,stroke-width:2px,color:#333333
    style CLIENTS fill:#FFFACD
    style BANKS fill:#FFFACD
```
---
